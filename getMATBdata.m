function [rate_data, sysmon_data, track_data, comm_data, resman_data, matb_data] = getMATBdata(data_folder)
%% Find MATB File
MATBregex = strcat(data_folder,'\','MATB_*.txt'); % 2022 for all original subjectsfullfile(data_folder,rate_file)
MATBfilename = dir(MATBregex);
MATBinfo = strcat(data_folder,'\',MATBfilename.name);
date_time = extractBetween(MATBfilename.name,'MATB_','.txt');
date_time = date_time{1};

%% RATE - get rate times
rate_file = strcat('RATE_',date_time,'.txt');
fid = fopen(fullfile(data_folder,rate_file));
...
fid = fopen(fullfile(data_folder,resman_file));
...
fid = fopen(fullfile(data_folder,comm_file));
...
fid = fopen(fullfile(data_folder,sysm_file));
...
fid = fopen(fullfile(data_folder,trck_file));
